...
export const useSignUp = routeAction$(async (params, { fail, redirect, ...req }) => {
    try {
        const user = await $trpc(req).user.create(params)
        throw redirect(303, `/sign-in?${new URLSearchParams({ email: user.email })}`)
    } catch (e) {
...
export default component$(() => {
    ...
    return (
        <>
            ...
            {action.value?.failed && <pre>{JSON.stringify(action.value, null, 2)}</pre>}
            <Form action={action}>
